...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Neural Style Transfer with Laplacian loss.')
    parser.add_argument('--method', type=str, default='default', choices=['default', 'laplacian'], 
                        help="Select the optimization method. Use 'laplacian' to include Laplacian loss.")
    parser.add_argument('--p', type=int, nargs='+', default=[2], 
                        help="List of average pooling sizes for Laplacian loss.")
    parser.add_argument('--laplacian_weights', type=float, nargs='+', default=[1000], 
                        help="List of weights corresponding to the average pooling sizes.")
...
optimization_config = {
    ...
    'method': args.method,
    'p': args.p,
    'laplacian_weights': args.laplacian_weights
}
...
if optimization_config['method'] == 'laplacian':
    if len(optimization_config['p']) != len(optimization_config['laplacian_weights']):
